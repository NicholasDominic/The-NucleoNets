...
def regularization(params, w, lambda_1, lambda_2):
    l1_norm = sum(p.abs().sum() for p in params)
    l2_norm = sum(p.pow(2.0).sum() for p in params)
    return w * ((lambda_1 * l1_norm) + (lambda_2 * l2_norm))
...
def fit(data, n_epochs, lr, batch_size, patience_num, reg_weight, lambda_1, lambda_2, *args, **kwargs):
    ...
    for epoch in range(1, n_epochs+1): # epoch
        ...
        for _, i in enumerate(train_dataloader): # mini batch
            ...
            # Forward Pass
            attn_score, y_predict = model(snp, pos, loc, var)
            attention_score_all_samples.append(attn_score)

            mse_score = mse_loss(y_predict, target)
            mse_per_batch.append(mse_score.item())
            
            loss = mse_score + regularization(model.parameters(), reg_weight, lambda_1, lambda_2)
        ...
        if mean(mse_per_batch) < best_train_loss:
            ...
            patience = patience_num
            if epoch > 500:
                torch.save(best_model.state_dict(), "model/v2-epoch-{}-mse-{:.5f}.pt".format(epoch, best_train_loss))
        else:
            patience -= 1
...
a, best_model, mse, mae, loss, early_stopping = fit(train_data, n_epochs=1000, lr=lr, batch_size=batch_size,
                                                    reg_weight=reg_w, lambda_1=lambda_1, lambda_2=lambda_2, patience_num=15)

df({
    'mse':mse,
    'early_stopping':early_stopping
}).to_csv("result/training_plot/v2-with-attn1-xavier-and-attn2-xavier.csv")
