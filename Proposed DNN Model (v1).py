...
def hyperopt(params):
    ...
    for f in range(folds):
        ...
        for data in validation_dataloader:
            with torch.no_grad():
                attn, output = model(data[0][0], data[0][1], data[0][2], data[0][3])
...
def fit(data, n_epochs, lr, batch_size, patience_num, *args, **kwargs):
    patience = patience_num
    best_model, best_train_loss = None, float("inf")
    mse_per_epoch, mae_per_epoch, r2_score_per_epoch = [], [], []
    ...
    for epoch in range(1, n_epochs+1): # epoch
        mse_per_batch, r2_score_per_batch, mae_per_batch = [], [], []
        ...
        if mean(mse_per_batch) < best_train_loss:
            ...
            patience = patience_num
            if epoch > 300:
                torch.save(best_model.state_dict(), "model/v1a-epoch-{}-mse-{:.5f}.pt".format(epoch, best_train_loss))
        else:
            patience -= 1
    ...
    return attention_score_all_samples, best_model, mse_per_epoch, mae_per_epoch, r2_score_per_epoch, early_stopping

a, best_model, mse, mae, r2, early_stopping = fit(train_data, n_epochs=1, lr=lr, batch_size=batch_size, patience_num=15)

# df({
#     'mse':mse,
#     'early_stopping':early_stopping
# }).to_csv("result/training_plot/v1-with-all-kaiming-uniform.csv")
