...
best_model.load_state_dict(torch.load("model/v3-epoch-979-mse-2.61582-attn1-xavier-attn2-normal.pt"))
...
def summarize(eval_model, test_data, threshold, *args, **kwargs):
    ...
    for i, data in enumerate(test_data):
        ...
        with torch.no_grad():
            _, attn, output = eval_model(test_dataloader[0][0], test_dataloader[0][1], test_dataloader[0][2], test_dataloader[0][3])
...
snp_table.to_csv("result/significant_snps/v3.csv")
snp_table
...
df({"threshold" : thresholds, "total_snps" : number_of_snps}).to_csv("result/significant_snps/v3-attn1-xavier-attn2-normal.csv")
...
evaluate(best_model, test_data[:15], "v3-plot-(sample.no.1-15).png", 1)
evaluate(best_model, test_data[15:30], "v3-plot-(sample.no.16-30).png", 16)
evaluate(best_model, test_data[30:45], "v3-plot-(sample.no.31-45).png", 31)
evaluate(best_model, test_data[45:60], "v3-plot-(sample.no.46-60).png", 46)
evaluate(best_model, test_data[60:75], "v3-plot-(sample.no.61-75).png", 61)
evaluate(best_model, test_data[75:90], "v3-plot-(sample.no.76-90).png", 76)
evaluate(best_model, test_data[90:], "v3-plot-(sample.no.91-104).png", 91)