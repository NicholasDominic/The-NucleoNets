...
space = {
    ...
    'reg_w': hp.choice('reg_w', [1/pow(10, l) for l in range(0, 5)])
}
...
space_list = {
    ...
    'reg_w': [1/pow(10, l) for l in range(0, 5)]
}
    ...
    for f in range(folds):
        ...
        for epoch in range(1, 20+1): # epoch
            for _, i in enumerate(train_dataloader): # mini batch
                ...
                # Backward Pass
                loss = mse_loss(y_predict, target) + regularization(model.parameters(), params["reg_w"], params["lambda_1"], params["lambda_2"])
                loss.backward()
                optimizer.step()
...
df({k : space_list[k][best[k]] for k in space_list}, index=["value"]).to_csv("result/v2-best_params.csv")
params = read_csv("result/v2a-best_params.csv", index_col=0)