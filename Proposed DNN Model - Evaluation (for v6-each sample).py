...
class SampleDataPreparation(torch.nn.Module):
    ...
    def forward(self, data, *args, **kwargs):
        if self.data_name == "Sample Variety":
            ...
            data = self.one_hot(data, num_classes=loc+1)
...
best_model.load_state_dict(torch.load("model/v6-xavier-best-model-params-regularization-entropy-epoch-73-mse-2.52197.pt"))
...
def evaluate(eval_model, test_data, SAVE_PATH, start_from, *args, **kwargs):
    plot_fig = plt.figure(figsize=(18, 250))
    ...
    for i, data in enumerate(test_data):
        ...
        plot_fig.add_subplot(35, 3, i+1) # position index always starts from 1, thus i+1
        test_dataloader = torch.utils.data.DataLoader(data)
        test_dataloader = list(test_dataloader)
        with torch.no_grad():
            attn, output = eval_model(test_dataloader[0][0], test_dataloader[0][1], test_dataloader[0][2])
            test_mse_loss = mse_loss(output, data[1]).item()
        
            # PLOT FIGURE
            attn_score = attn.tolist()
            plt.scatter(snps_pos[i], attn_score)
            plt.ylim(-.1, 1.1)
            plt.xlabel("SNPs")
    ...
    plt.savefig("result/" + SAVE_PATH)
    plt.tight_layout() # margin adjusted

evaluate(best_model, test_data[:52], "v6-manhattan_plot1.png", 0)

evaluate(best_model, test_data[52:], "v6-manhattan_plot2.png", 52)